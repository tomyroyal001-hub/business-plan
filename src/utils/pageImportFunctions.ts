...
export const importCoverPageData = (data: PageImportData) => {
  return {
    ...
    // Dynamic stats cards
    statsCards: data.statsCards ? JSON.parse(data.statsCards) : [
      { label: 'Founded', value: '2024', color: 'purple' },
      { label: 'Team', value: '10+', color: 'blue' },
      { label: 'Markets', value: '5', color: 'green' },
      { label: 'Potential', value: '∞', color: 'orange' }
    ] as StatsCard[]
  };
};
...
export const importMarketAnalysisData = (data: PageImportData) => {
  return {
    ...
    // Dynamic market data
    totalMarket: data.totalMarket || '$2.5B',
    marketGrowth: data.marketGrowth || '25% annually',
    marketSegments: data.marketSegments ? JSON.parse(data.marketSegments) : [
      { name: 'Primary Market', value: '$800M', percentage: '65%', color: 'from-green-400 to-blue-500' },
      { name: 'Secondary Market', value: '$400M', percentage: '35%', color: 'from-purple-400 to-pink-500' }
    ] as MarketSegment[]
  };
};
...
export const importOrganizationManagementData = (data: PageImportData) => {
  return {
    ...
    stakeholders: data.stakeholders ? JSON.parse(data.stakeholders) : [
      { id: 1, name: 'John Smith', role: 'CEO & Co-Founder', stake: '45%', experience: '12+ years', email: 'john@techventure.com', linkedin: 'linkedin.com/in/johnsmith-tech' },
      { id: 2, name: 'Sarah Johnson', role: 'CTO & Co-Founder', stake: '35%', experience: '10+ years', email: 'sarah@techventure.com', linkedin: 'linkedin.com/in/sarahjohnson-ai' },
      { id: 3, name: 'Alex Thompson', role: 'Lead Engineer', stake: '5%', experience: '8+ years', email: 'alex@techventure.com', linkedin: 'linkedin.com/in/alexthompson-eng' },
      { id: 4, name: 'Maria Garcia', role: 'Head of Customer Success', stake: '3%', experience: '6+ years', email: 'maria@techventure.com', linkedin: 'linkedin.com/in/mariagarcia-cs' }
    ] as Stakeholder[]
  };
};
...
export const importProductServiceData = (data: PageImportData) => {
  return {
    ...
    productFeatures: data.productFeatures ? JSON.parse(data.productFeatures) : [
      { name: 'Innovation', description: 'Cutting-edge solutions', percentage: '90%', color: 'from-yellow-400 to-orange-500', icon: 'Zap' },
      { name: 'Quality', description: 'Premium standards', percentage: '95%', color: 'from-green-400 to-blue-500', icon: 'Shield' }
    ] as ProductFeature[]
  };
};
...
export const importMarketingSalesData = (data: PageImportData) => {
  return {
    ...
    marketingChannels: data.marketingChannels ? JSON.parse(data.marketingChannels) : [
...
export const importFinancialProjectionsData = (data: PageImportData) => {
  return {
    ...
    financialMetrics: data.financialMetrics ? JSON.parse(data.financialMetrics) : [
      { label: 'Gross Margin', value: '25%', description: 'Strong profitability', percentage: '75%', color: 'from-green-400 to-emerald-500' },
      { label: 'Break-even', value: '18', description: 'Months to Break-even', percentage: '60%', color: 'from-blue-400 to-purple-500' },
      { label: 'ROI', value: '3.2x', description: 'ROI (5 years)', percentage: '85%', color: 'from-yellow-400 to-orange-500' }
    ] as FinancialMetric[],
    projectedRevenue: data.projectedRevenue || '$2.5M',
    revenueTimeline: data.revenueTimeline || 'by Year 3'
  };
};
...
export const importFundingRequestData = (data: PageImportData) => {
  return {
    ...
    fundingAllocation: data.fundingAllocation ? JSON.parse(data.fundingAllocation) : [
      { category: 'Product Development', amount: '$800K', percentage: '40%', color: 'from-blue-400 to-blue-600', icon: '🚀' },
      { category: 'Marketing & Sales', amount: '$700K', percentage: '35%', color: 'from-purple-400 to-purple-600', icon: '📈' },
      { category: 'Personnel', amount: '$400K', percentage: '20%', color: 'from-green-400 to-green-600', icon: '👥' },
      { category: 'Operations', amount: '$100K', percentage: '5%', color: 'from-yellow-400 to-yellow-600', icon: '⚙️' }
...
export const importGrantsData = (data: PageImportData) => {
  return {
    ...
    fundingStats: data.fundingStats ? JSON.parse(data.fundingStats) : [
      { label: 'Gov Grants', value: '15+', color: 'orange' },
      { label: 'NGO Grants', value: '8+', color: 'green' },
      { label: 'Max Grant', value: '₹50L', color: 'blue' }
    ] as StatsCard[]
  };
};
